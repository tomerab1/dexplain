import { CATEGORY, SEVERITY, THRESHOLDS } from '../constants.mjs';
import { formatBytes, formatDuration } from '../format.mjs';
import { colorizer } from './colors.mjs';
import { renderTimeline, shouldShowTimeline } from './timeline.mjs';
...
/** True when the timeline should render: forced on, or auto and the build is big enough. */
function timelineWanted(report, timeline) {
  if (!report.buildTrace) return false;
  if (timeline === false) return false;
  return timeline === true || shouldShowTimeline(report.buildTrace, THRESHOLDS);
}

/** Renders a report as the default terminal summary. `color` gates ANSI styling. */
export function renderHuman(report, { color = true, timeline = null, width = null } = {}) {
  ...
  if (timelineWanted(report, timeline)) {
    sections.push(renderTimeline(report.buildTrace, { paint, width: width ?? undefined }).join('\n'), '');
  }
