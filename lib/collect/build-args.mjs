...
// Build context identifiers
const STDIN_CONTEXT = '-';
const URL_PROTOCOL_MARKER = '://';
const GIT_SSH_PREFIX = 'git@';

/**
 * Default isDir implementation using node:fs.statSync.
 * @param {string} path - Path to check.
 * @returns {boolean} True if path is an existing directory.
 */
...
export function parseBuildArgs(argv, { isDir = defaultIsDir } = {}) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    const token = argv[i];
    // Treat "-" as a positional (stdin), not a flag
