...
export function parseCliArgs(argv) {
  const options = { json: false, noColor: false, jsonOut: null, image: null, top: null, failOn: null };
  const remaining = [];
...
async function collectAnalyze(rest, options, io) {
  const path = rest[0];
...
async function collectImageCommand(rest, options, io) {
  const ref = rest[0];
...
function collectDockerfile(rest, options, io) {
