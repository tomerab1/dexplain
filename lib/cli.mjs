...
import { DEFAULT_DOCKERFILE, EXIT } from './constants.mjs';
import { dockerStatus } from './docker.mjs';
...
import { buildReport } from './model/report.mjs';
...
const VALUE_OWN_FLAGS = new Set(['--json-out', '--top', '--image']);
...
export function parseCliArgs(argv) {
  const options = { json: false, noColor: false, jsonOut: null, image: null, top: null };
  const remaining = [];
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (flag === '--json' || flag === '--no-color') assignFlag(options, flag);
    else remaining.push(token);
  }
  return { command: remaining[0] ?? null, rest: remaining.slice(1), options };
}

async function collectBuild(rest) {
  const result = await runInstrumentedBuild(rest);
...
function emit(report, options, io) {
  ...
  if (options.json) io.out(full);
    ...
    io.out(renderHuman(forDisplay, { color: options.color }));
  }
  if (options.jsonOut) {
