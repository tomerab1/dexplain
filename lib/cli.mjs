...
import { DEFAULT_DOCKERFILE, EXIT, SEVERITY, SEVERITY_RANK } from './constants.mjs';
...
import { renderDiffHuman } from './render/diff-human.mjs';
import { createProgressPrinter } from './render/progress.mjs';
import { diffLayers } from './diff/layers.mjs';
import { diffFindings } from './diff/findings.mjs';
import { diffTiming } from './diff/timing.mjs';
...
Options:
  ...
  --trace-a <file>    (diff) attach a build trace to image side A
  --trace-b <file>    (diff) attach a build trace to image side B
...
const VALUE_OWN_FLAGS = new Set(['--json-out', '--top', '--image', '--trace-a', '--trace-b', '--fail-on']);
...
function assignFlag(options, flag, value) {
  ...
  else if (flag === '--trace-a') options.traceA = value;
  else if (flag === '--trace-b') options.traceB = value;
  else if (flag === '--top') options.top = Number(value);
  else if (flag === '--fail-on') options.failOn = value;
}

export function parseCliArgs(argv) {
  const options = { json: false, noColor: false, jsonOut: null, image: null, traceA: null, traceB: null, top: null, failOn: null, timeline: null };
  const remaining = [];
...
function resolveSideSpec(spec) {
  if (existsSync(spec) && spec.endsWith('.ndjson')) return { type: 'trace', path: spec };
  return { type: 'image', ref: spec };
}

function collectSideTrace(spec, traceOption) {
  if (spec.type === 'trace') {
    if (!existsSync(spec.path)) return { fatal: { reason: `trace file not found: ${spec.path}`, code: EXIT.RUNTIME_ERROR } };
    return { trace: spec.path, buildTrace: parseBuildTrace(readFileSync(spec.path, 'utf8')) };
  }
  if (traceOption) {
    if (!existsSync(traceOption)) return { fatal: { reason: `trace file not found: ${traceOption}`, code: EXIT.RUNTIME_ERROR } };
    return { buildTrace: parseBuildTrace(readFileSync(traceOption, 'utf8')) };
  }
  return {};
}

async function collectSideImage(spec, warnings) {
  if (spec.type !== 'image') return { image: null };
  const img = await collectImage(spec.ref);
  if (img.error) {
    warnings.push(img.error);
    return { image: null };
  }
  return { image: img.image };
}

async function collectDiff(rest, options) {
  const refA = rest[0];
  const refB = rest[1];
  if (!refA || !refB) return { fatal: { reason: 'usage: dexplain diff <a> <b>', code: EXIT.USAGE } };

  const warnings = [];
  const specA = resolveSideSpec(refA);
  const specB = resolveSideSpec(refB);

  const traceResultA = collectSideTrace(specA, options.traceA);
  if (traceResultA.fatal) return traceResultA;
  const traceResultB = collectSideTrace(specB, options.traceB);
  if (traceResultB.fatal) return traceResultB;

  const needsDocker = specA.type === 'image' || specB.type === 'image';
  if (needsDocker) {
    const status = await dockerStatus();
    if (!status.ok) return { fatal: { reason: status.reason, code: EXIT.DOCKER_UNAVAILABLE } };
  }

  const imgA = await collectSideImage(specA, warnings);
  const imgB = await collectSideImage(specB, warnings);

  const sideA = { ...traceResultA, ...(specA.type === 'image' && { ref: specA.ref }), ...imgA };
  const sideB = { ...traceResultB, ...(specB.type === 'image' && { ref: specB.ref }), ...imgB };

  const findingsA = runRules(sideA).findings;
  const findingsB = runRules(sideB).findings;

  const deltas = { findings: diffFindings(findingsA, findingsB) };
  if (imgA.image && imgB.image) deltas.layers = diffLayers(imgA.image, imgB.image);
  else if ((imgA.image || imgB.image) && !(imgA.image && imgB.image)) warnings.push('layer delta skipped: only one side is an image');

  if (sideA.buildTrace && sideB.buildTrace) deltas.timing = diffTiming(sideA.buildTrace, sideB.buildTrace);
  else if ((sideA.buildTrace || sideB.buildTrace) && !(sideA.buildTrace && sideB.buildTrace)) warnings.push('timing delta skipped: only one side has a build trace');

  const diff = {
    a: { ...(sideA.ref && { ref: sideA.ref }), ...(sideA.trace && { trace: sideA.trace }), findings: findingsA },
    b: { ...(sideB.ref && { ref: sideB.ref }), ...(sideB.trace && { trace: sideB.trace }), findings: findingsB },
    deltas,
  };

  return { command: 'diff', diff, warnings, exitCode: EXIT.OK };
}
...
const COMMANDS = { build: collectBuild, analyze: collectAnalyze, image: collectImageCommand, dockerfile: collectDockerfile, diff: collectDiff };
...
function emitDiff(diff, options, io) {
  if (options.json) io.out(renderJson(diff));
  else io.out(renderDiffHuman(diff, { color: options.color }));
  if (options.jsonOut) {
    writeFileSync(options.jsonOut, renderJson(diff));
    io.err(`report written to ${options.jsonOut}`);
  }
}

function introducedFindingsAboveSeverity(diff, severity) {
  const severityRank = SEVERITY_RANK[severity];
  if (severityRank === undefined) return false;
  for (const finding of diff.deltas.findings.introduced) {
...
export async function main(argv, io = defaultIo()) {
  ...
  if (collected.diff) {
    emitDiff(collected.diff, options, io);
    if (options.failOn && collected.exitCode === EXIT.OK && introducedFindingsAboveSeverity(collected.diff, options.failOn)) {
