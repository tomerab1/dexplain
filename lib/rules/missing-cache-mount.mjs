...
import { commandTextOf, instructionHasCacheMount, matchPackageManager, runInstructions } from './helpers.mjs';
...
export default {
  ...
  evaluate(model) {
    ...
    for (const instruction of runInstructions(model.dockerfile)) {
      ...
      const manager = matchPackageManager(text);
      if (manager && !instructionHasCacheMount(instruction)) findings.push(toFinding(instruction, manager));
    }
    return findings;
  },
};
