import { BUILD_COMMAND, CATEGORY, FIX_RISK, INPUT, SEVERITY } from '../constants.mjs';
import { makeFinding } from '../model/finding.mjs';
import { commandTextOf, matchPackageManager, runInstructions } from './helpers.mjs';
...
function buildsArtifacts(dockerfile) {
  return runInstructions(dockerfile).some((instruction) => {
    const text = commandTextOf(instruction);
    return BUILD_COMMAND.test(text) || matchPackageManager(text);
  });
}
