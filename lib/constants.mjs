...
export const PACKAGE_MANAGER_CACHE_MOUNTS = Object.freeze([
  ...
  { name: 'bundler', match: /\bbundle\s+install\b/, target: '/usr/local/bundle/cache' },
  { name: 'poetry', match: /\bpoetry\s+(?:-{1,2}\S+\s+)*install\b/, target: '/root/.cache/pypoetry' },
  { name: 'uv', match: /\buv\s+pip\s+(?:-{1,2}\S+\s+)*install\b|\buv\s+sync\b/, target: '/root/.cache/uv' },
  ...
  { name: 'cargo', match: /\bcargo\s+(?:-{1,2}\S+\s+)*(build|install|fetch)\b/, target: '/usr/local/cargo/registry' },
  { name: 'composer', match: /\bcomposer\s+(?:-{1,2}\S+\s+)*(install|require|update)\b/, target: '/root/.composer/cache' },
  { name: 'gradle', match: /(?:\.\/)?gradlew\b\s+(?:-{1,2}\S+\s+)*(build|assemble|test)\b|\bgradle\s+(?:-{1,2}\S+\s+)*(build|assemble|test)\b/, target: '/root/.gradle/caches' },
  { name: 'maven', match: /\bmvn\s+(?:-{1,2}\S+\s+)*(package|install|verify|deploy|compile|test)\b/, target: '/root/.m2/repository' },
